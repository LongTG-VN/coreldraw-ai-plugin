...
details{max-width:820px;margin:12px auto;background:var(--panel);padding:11px 14px;border-radius:8px}
...
.overall-quality{max-width:820px;margin:12px auto;background:#2c2922;border:1px solid var(--accent);padding:12px 14px;border-radius:8px;display:grid;grid-template-columns:1fr 130px;align-items:center;gap:16px}.overall-quality strong{display:block;margin-bottom:3px}
...
<div class="overall-quality"><div><strong>Chất lượng tổng thể của cặp thiết kế</strong><span class="muted">Tùy chọn · 1 rất yếu, 10 đủ thuyết phục</span></div><select id="overallQuality"><option value="">—</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option></select></div>
<details><summary>Chấm chi tiết tùy chọn</summary><div class="scores" id="scores"></div><textarea id="notes" maxlength="2000" placeholder="Ghi chú ngắn (không bắt buộc)"></textarea><label class="muted">Độ tự tin (không bắt buộc)<select id="confidence"><option value="">—</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option></select></label></details>
...
<script>
const byId=id=>document.getElementById(id);const login=byId('login'),loginForm=byId('loginForm'),reviewer=byId('reviewer'),reviewerLabel=byId('reviewerLabel'),brief=byId('brief'),category=byId('category'),imageA=byId('imageA'),imageB=byId('imageB'),progress=byId('progress'),scores=byId('scores'),overallQuality=byId('overallQuality'),notes=byId('notes'),confidence=byId('confidence'),message=byId('message'),chooseA=byId('chooseA'),chooseB=byId('chooseB'),tie=byId('tie'),bad=byId('bad'),skip=byId('skip'),back=byId('back'),zoom=byId('zoom'),zoomImage=byId('zoomImage');
let sessionId=null,current=null,busy=false;const dimensions=['composition','hierarchy','typography','brand_feeling'];
const labels={composition:'Composition',hierarchy:'Hierarchy',typography:'Typography',brand_feeling:'Brand feeling'};
...
function msg(v){message.textContent=v}function clearOptional(){document.querySelectorAll('[data-score]').forEach(x=>x.value='');overallQuality.value='';notes.value='';confidence.value=''}
...
async function submit(choice){if(busy||!current)return;busy=true;try{const score={};if(overallQuality.value)score.overall_quality=Number(overallQuality.value);document.querySelectorAll('[data-score]').forEach(x=>{if(x.value)score[x.dataset.score]=Number(x.value)});await api('/api/v1/review/submit',{method:'POST',body:JSON.stringify({session_id:sessionId,pair_id:current.pair_id,review:{choice,scores:Object.keys(score).length?score:null,notes:notes.value||null,confidence:confidence.value?Number(confidence.value):null}})});await loadNext()}catch(e){msg(e.message)}finally{busy=false}}
